...
- **Modeling**: Full geometric and meshing capabilities. The electrode montage identified during Classic TI optimization can be reconstructed and adjusted on the subject model.
...
| Input | Contents |
| ----- | -------- |
| `input_1/` | TI and HF field `.cache` files exported from Classic TI via `Export to S4L` |
...
The subject model forwarded to Exposure Analysis is determined by the previous step: once a subject is selected in the Classic TI dropdown and **Load** is clicked, the corresponding `.smash` file (and DTI tensors, where available) are automatically forwarded.

### Typical Workflow

1. Run the SuMo optimizer in [Classic TI](/docs/services/post_processing.md) to identify the best electrode configurations.
...
- A pre-configured template multiport simulation that can be used to:
  ...
  - **Investigate a specific configuration** by adapting the montage identified during Classic TI optimization.
